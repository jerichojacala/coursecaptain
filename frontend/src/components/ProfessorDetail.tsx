...
export default function ProfessorDetailView({ id }: { id: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/professors/${id}/`, { 
          cache: 'no-store' 
        });
        ...
        const jsonData: Professor = await res.json();
        setData(jsonData);
      } catch (err) {
        setError(true);
      }
  ...
  return (
    <div className="border-2 border-solid border-gray-800 p-8 max-w-6xl mx-auto">
      <div className="flex flex-row">
        <h1 className="text-3xl font-bold m-2">{data.first_name} {data.last_name}</h1>
        <h1 className="text-gray-600 text-xl m-4 italic">{data.school.name}</h1>
      </div>
